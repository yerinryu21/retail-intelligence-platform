...
**Total expected revenue at risk:**  Summing CLV × ChurnProbability across all 784 customers gives $394,552.83 — an expected-value figure, distinct from Week 2's threshold-based revenue-at-risk ($559,896.14), which summed full CLV only for the 603 customers crossing the 0.20 decision threshold. The two numbers answer different questions: Week 2's figure estimates loss if every flagged customer churned outright; this figure weights each customer's contribution by their actual predicted likelihood. Both are valid depending on framing — the app and downstream reporting use this expected-value figure as the primary metric.
